...
interface MarkdownFormProps {
  onSubmit: (markdownText: string, options: FormOptions) => Promise<void>;
...
export default function MarkdownForm({
  ...
  const [preserveUnicode, setPreserveUnicode] = useState(true);
  ...
  return (
    <form className="app-form" onSubmit={handleSubmit}>
      ...
      <OptionsPanel
        ...
        onPreserveUnicodeChange={(checked) => setPreserveUnicode(checked)}
      />

      <button
        type="submit"
        className={`submit-button mt-4 bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-6 rounded transition duration-300 w-full flex justify-center items-center ${
          isLoading ? "is-loading opacity-75 cursor-not-allowed" : ""
        }`}
        disabled={isLoading}
      >
        {isLoading ? (
