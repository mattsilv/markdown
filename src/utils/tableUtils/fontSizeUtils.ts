...
export function normalizeTableFontSizes(container: HTMLElement): number | null {
  ...
  const tableSizeNeeds = Array.from(tables).map((table) =>
    analyzeTableSizeNeeds(table)
  );
...
function findSmallestRequiredFontSize(tableSizeNeeds: TableAnalysis[]): number {
  ...
  const minFontSize = Math.min(
    ...tableSizeNeeds.map((analysis) => analysis.recommendedFontSize)
  );
 ...
 */
function applyNormalizedFontSize(
 ...
 */
function applyConsistentTableStyling(
  tables: NodeListOf<HTMLTableElement>
): void {
  tables.forEach((table) => {
    ...
    // Force table to use table display for consistency between HTML and print views
    ...
    if (headerRow) {
      ...
      headerCells.forEach((cell) => {
        cell.style.backgroundColor = "#f2f7fd"; // Light blue
        cell.style.color = "#333333"; // Dark text for contrast
        cell.style.fontWeight = "600"; // Semi-bold
        cell.style.textAlign = "center";
        cell.style.position = "relative"; // Not sticky for consistency
      });
    }
    ...
    const headers = Array.from(
      table.querySelectorAll<HTMLElement>("tr:first-child th")
    );
    ...
    if (idColumnIndex !== -1) {
      ...
      // Style ID cells
      const idCells = table.querySelectorAll<HTMLElement>(
        `tr td:nth-child(${idColumnIndex + 1})`
      );
      ...
      // Style ID header
      const idHeader = table.querySelector<HTMLElement>(
        `tr:first-child th:nth-child(${idColumnIndex + 1})`
      );
      if (idHeader) {
    ...
    // Add zebra striping
    const rows = table.querySelectorAll<HTMLElement>("tr:nth-child(even)");
    rows.forEach((row) => {
